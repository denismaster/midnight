...
import net.sf.l2j.gameserver.scripting.quests.Tutorial;
...
public class EnterWorld extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
	 ...
	 try{
		...
		_log.warning("EnterWorld: loadTutorial");
        QuestState qs = activeChar.getQuestState("Tutorial");
        if(qs != null){
			try{
            qs.getQuest().notifyEvent("UC", null, activeChar);
			qs.getQuest().onAdvEvent("UC", null, activeChar);
			_log.warning("EnterWorld:  qs.getQuest ="+ qs.getQuest()+"");
			_log.warning("EnterWorld: loadTutorial not NULL");
			}
			catch(Exception e)
			{
			_log.log(Level.WARNING,"EnterWorld: loadTutorial is NULL",e);}
		}
		else
			_log.warning("EnterWorld: loadTutorial is NULL");
		}
	catch(Exception e)
	{_log.log(Level.WARNING,"EnterWorld: loadTutorial is NULL",e);}
	}
