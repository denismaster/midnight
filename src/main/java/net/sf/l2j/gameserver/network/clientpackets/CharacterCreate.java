...
public final class CharacterCreate extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		for (L2SkillLearn skill : SkillTreeTable.getInstance().getAvailableSkills(newChar, newChar.getClassId()))
		{
		  ...
		  if(!Config.DISABLE_TUTORIAL && newChar.getQuestState("Tutorial") == null)
        {
			
            Quest q = ScriptManager.getInstance().getQuest("Tutorial");
            if(q != null){
                q.newQuestState(newChar).setState((byte)1);
						_log.warning("Q255 registered for new char");
			try{
			QuestState qt = newChar.getQuestState("Tutorial");
			_log.warning("Q255 qt ravno newcchar queststate");
			qt.set("ucMemo","0");
			_log.warning("Q255 ucMemo setted to 0");
			}
			catch(Exception e)
			{
				}
			...
			_log.warning("Q255:CharacterCreate не сработало");
